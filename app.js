// ============================================================================
// APPÃ€RE - Dashboard Benchmark Allevatore
...
/**
 * Mappa dei nomi KPI logici â†’ alias presenti nel dataset.
...
/**
 * UnitÃ  di misura per ciascun KPI, utilizzata nelle label degli assi.
 */
const KPI_UNITS = {
  cellule:   'cell/mL',
  carica:    'UFC/mL',
...
/**
 * Etichette dei mesi per le lattazioni (Ottobre â†’ Settembre).
 */
const LAC_MONTHS_IT = [
 ...
 *     - 'lactation': una lattazione intera (Ottâ†’Set), start = anno di inizio
...
/**
 * Flag per auto-selezionare la lattazione piÃ¹ recente alla prima inizializzazione.
 ...
 * - provincia (filtro sul gruppo, ma l'azienda selezionata Ã¨ sempre inclusa)
 *
 * NOTA:
...
function getBenchmarkRaw() {
  ...
  // IntraCaseificio â†’ filtra solo i record del caseificio dell'azienda selezionata
  ...
  // ModalitÃ  'intraAppare' e 'regione':
...
/**
 * Per alcuni KPI un valore piÃ¹ basso Ã¨ migliore (cellule, carica).
 */
...
/**
 * Mostra, in modalitÃ  IntraCaseificio, il nome del caseificio
...
// ============================================================================
// CACHE YM (anno-mese â†’ Map(Azienda, valore aggregato))
// ============================================================================
...
/**
 * Anno di inizio della lattazione corrente (Ottâ†’Set).
 */
...
/**
 * Le ultime tre lattazioni basate sulla data odierna (non usato piÃ¹ qui, mantenuto per completezza).
 */
...
/**
 * Posizione (0..11) di un mese (0..11) all'interno della lattazione (Ottâ†’Set).
 * Ott(9) â†’ 0, Nov(10) â†’ 1, ..., Set(8) â†’ 11
 */
...
function ensureAziendaSelector() {
  try {
    ...
    // Ricostruisci options solo se diverso da quello giÃ  presente
    ...
    if (!sel._bound) {
      sel.addEventListener('change', () => {
        ...
        // YM dipende anche dall'azienda â†’ svuota la cache
...
/**
 * Aggiorna il grafico PR (linea 0â€“100%) per le lattazioni selezionate.
...
function updatePR(rows) {
  ...
  // Mappa checkbox (yr2023, yr2024, yr2025) â†’ anno di inizio lattazione
  ...
  map.forEach(([id, yStart]) => {
    ...
    if (!yStart || !inp || !lab) {
      // Nessuna lattazione associata â†’ nascondi/azzera
  ...
  for (var yStart of selected) {
    ...
    // Ottobreâ€“Dicembre dell'anno yStart
    for (var m = 9; m <= 11; m++) {
    ...
    // Gennaioâ€“Settembre dell'anno successivo
...
function updateKPI(rows) {
  ...
  for (var yStart of selected) {
    ...
    // Serie azienda
    datasets.push({
      label: lactationLabel(yStart) + ' â€“ KPI',
      data: azi,
      borderColor: c,
    ...
    // Serie mediana
    datasets.push({
      label: lactationLabel(yStart) + ' â€“ Mediana',
      data: med,
      borderColor: c,
...
function updateHistogram(rows) {
  ...
  if (!ymKeys.length) {
    ...
    if (pbEmpty) pbEmpty.textContent = 'â€”Â° percentile';
    return;
  }
  ...
  if (!vals.length) {
    ...
    if (pbNo) pbNo.textContent = 'â€”Â° percentile';
    return;
  }
  ...
  if (posBadge) {
    posBadge.textContent = (pr != null) ? (pr + '° percentile') : 'â€”° percentile';
  }
}
 ...
 *        - false â†’ forza la selezione sull'ultima lattazione disponibile
 *        - true  â†’ se possibile mantiene la lattazione/custom giÃ  selezionati
 */
function rebuildLactationMenu(preserveSelection = false) {
  ...
  if (preserveSelection) {
    ...
    if (previousType === 'custom') {
      ...
      // state.histPeriod (from/to) resta com'Ã¨
      return;
    }
    ...
    // Se arrivo qui, non posso preservare â†’ vado in fallback sotto
  }

  // Fallback standard: se ci sono lattazioni â†’ ultima, altrimenti custom vuoto
  if (lacStarts.length) {
 ...
 * includendo eventualmente le estremitÃ  dell'intervallo (from/to).
 */
  ...
  document.addEventListener('raw:loaded', function () {
    try {
      ...
      if (kSelLoaded) {
        ...
        // Fallback se il select KPI non Ã¨ presente
