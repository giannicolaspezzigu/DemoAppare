  ...
  function renderHistogram() {
    ...
    chart.options.plugins.tooltip = {
      ...
      callbacks: {
        label(ctx) {
          const d = ctx.raw || {};
          const pct = Number.isFinite(d.y) ? d.y.toFixed(1) + '%' : '';
          const cnt = Number.isFinite(d.count) ? d.count + ' aziende' : '';
          const range = (Number.isFinite(d.from) && Number.isFinite(d.to))
            ? 'Range: [' + d.from.toFixed(2) + ' ; ' + d.to.toFixed(2) + ']'
            : '';
